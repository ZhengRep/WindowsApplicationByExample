...
public:
	EllipseFigure();
	EllipseFigure(const Color& color, const CPoint& ptTopLeft, BOOL bFilled);
	EllipseFigure(const EllipseFigure& ellipse);
	Figure* Copy() const;
	void Serialize(CArchive& archive) { return RectangleFigure::Serialize(archive); };
	HCURSOR GetCursor() const;
	BOOL Click(const CPoint& ptMouse);
	BOOL DoubleClick(const CPoint& ptMouse) { return RectangleFigure::DoubleClick(ptMouse); };
	BOOL Inside(const CRect& rcInside) const { return RectangleFigure::InSide(rcInside); };
	void Move(const CSize& szDistance) { return RectangleFigure::Move(szDistance); };
